...
A simple point-of-sale web app: customers can order as a guest or log in; admins and staff manage the menu and orders. Built with Node.js and MySQL.
...
**Optional:** Do the same for **seed_menu.sql** (adds the default menu) and **set_roles.sql** (sets up admin/staff roles). Paste each file’s contents into the SQL tab and click **Go**.
...
Only Admin can see the admin panel. Everyone else can place orders as a guest.
...
| **set_roles.sql** | Set admin/staff roles and staff user (optional). |
